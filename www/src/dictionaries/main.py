...
def translate_and_save_language(lang_code):
    ...
    try:
        ...
        translated_texts_list = translator.translate_batch(
            texts_to_translate_list)

        if translated_texts_list is None or len(translated_texts_list) != len(
                texts_to_translate_list):
            error_msg = (
                f"Batch translation for {lang_code} returned an unexpected result. "
                f"Expected {len(texts_to_translate_list)} segments, "
                f"got {len(translated_texts_list) if translated_texts_list else 'None'}."
            )
            print(f"  Warning: {error_msg}")
            print(
                f"  Falling back to saving original English text for {lang_code}.json"
            )
            ...
            for i, string_info in enumerate(original_strings_info):
                current_lang_flat_dict[
                    string_info['key']] = translated_texts_list[i]
    ...
    try:
        with open(f"{lang_code}.json", "w", encoding="utf-8") as out_file:
            json.dump(translated_json_content,
                      out_file,
                      ensure_ascii=False,
                      indent=2)
